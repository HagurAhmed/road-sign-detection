...
def build_r_table(image, origin):
    ...
    '''
    edges = feature.canny(image, low_threshold=MIN_CANNY_THRESHOLD,
...
def general_hough_closure(reference_image):
    ...
    '''
    referencePoint = (int(reference_image.shape[0] / 2), int(reference_image.shape[1] / 2))
...
def test_general_hough(gh, reference_image, query,frame):
    '''
    Uses a GH closure to detect shapes in an image and create nice output
    '''
    #query_image=cv2.imread(query,1)
    #query_image = imread(query, flatten=True)

    accumulator = gh(cv2.cvtColor(query, cv2.COLOR_BGR2GRAY))
    # print("hagur")
    # plt.clf()
    # plt.gray()
    # print("hagur2")
    ...
    # print(query_image.shape,x_points,y_points,accumulator.shape)
    ...
    #plt.show()
    cv2.rectangle(frame,(i-1,j-1),(i+1,j+1),(255,80,255),3)
    cv2.rectangle(query, (i - 1, j - 1), (i + 1, j + 1), (255, 80, 255), 3)
    frame= cv2.resize(frame,(600,600))
    query= cv2.resize(query,(600,600))
    print(i,j)
    return np.hstack([frame,query])


def test():
    reference_image = cv2.imread('cl23.png')
    # reference_image = img
    reference_image = cv2.resize(reference_image,(90,90))
    reference_image = cv2.medianBlur(reference_image, 5)
    cv2.imshow("hh",reference_image)
    cv2.waitKey(0)
    #reference_image = imread('triangle_S2.png', flatten=True)
    reference_image = cv2.cvtColor(reference_image, cv2.COLOR_BGR2GRAY)
    detect_s = general_hough_closure(reference_image)

    i=2300
    c = cv2.imread("test.png",1)
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            mask2 = cv2.inRange(hsv, np.array([160, 100, 100]), np.array([179, 255, 255]))  # upper red
            mask3 = cv2.inRange(hsv, np.array([100, 150, 0]), np.array([140, 255, 255]))  # upper blue
            mask = mask1 + mask2 + mask3
            img = cv2.bitwise_and(img, img, mask=mask)
            img = cv2.medianBlur(img, 5)

            g=test_general_hough(detect_s, reference_image, img,frame)
            cv2.imshow("out",g)
